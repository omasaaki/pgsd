...
class TestSchemaInformationCollector:
    ...
    @pytest.mark.asyncio
    async def test_collect_schema_info_success(self, schema_collector, mock_database_manager,
                                             mock_connector, sample_table_data, sample_column_data):
    ...
    @pytest.mark.asyncio
    async def test_collect_schema_info_cached(self, schema_collector, mock_database_manager,
                                            mock_connector, sample_table_data, sample_column_data):
    ...
    @pytest.mark.asyncio
    async def test_collect_schema_info_permission_error(self, schema_collector, mock_database_manager,
                                                       mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_collect_schema_info_query_error(self, schema_collector, mock_database_manager,
                                                  mock_connector):
        """Test schema collection with query error."""
        # Setup mocks
        mock_database_manager.get_source_connection = AsyncMock(return_value=mock_connector)
        mock_database_manager.return_source_connection = Mock()
        mock_connector.execute_query = AsyncMock(side_effect=DatabaseQueryError("Query failed"))
    ...
    @pytest.mark.asyncio
    async def test_collect_tables(self, schema_collector, mock_database_manager,
                                mock_connector, sample_table_data, sample_column_data):
    ...
    @pytest.mark.asyncio
    async def test_collect_columns(self, schema_collector, mock_connector, sample_column_data):
        """Test column collection."""
        # Setup mock
        mock_connector.execute_query = AsyncMock(return_value=sample_column_data)
        
        # Test column collection
        columns = await schema_collector._collect_columns(mock_connector, "public", "users")
    ...
    async def test_collect_constraints(self, schema_collector, mock_connector):
        ...
        # Test constraint collection
        constraints = await schema_collector._collect_constraints(mock_connector, "public")
    ...
    def test_cache_functionality(self, schema_collector):
        ...
        schema_collector._cache_timestamps["source:public"] = datetime.utcnow() - timedelta(seconds=400)
        assert not schema_collector._is_cached("source:public")
    ...
    @pytest.mark.asyncio
    async def test_get_available_schemas(self, schema_collector, mock_database_manager, mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_target_database_collection(self, schema_collector, mock_database_manager, mock_connector):
        """Test schema collection for target database."""
        # Setup mocks
        mock_database_manager.get_target_connection = AsyncMock(return_value=mock_connector)
