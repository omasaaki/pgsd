...
class TestVersionDetector:
    ...
    @pytest.mark.asyncio
    async def test_detect_versions_success(self, version_detector, mock_database_manager, 
                                         mock_source_connector, mock_target_connector):
        """Test successful version detection."""
        # Setup mocks
        mock_database_manager.get_source_connection = AsyncMock(return_value=mock_source_connector)
        mock_database_manager.get_target_connection = AsyncMock(return_value=mock_target_connector)
    ...
    @pytest.mark.asyncio
    async def test_detect_versions_cached(self, version_detector, mock_database_manager,
                                        mock_source_connector, mock_target_connector):
    ...
    @pytest.mark.asyncio
    async def test_detect_versions_force_refresh(self, version_detector, mock_database_manager,
                                                mock_source_connector, mock_target_connector):
    ...
    @pytest.mark.asyncio
    async def test_detect_versions_source_failure(self, version_detector, mock_database_manager):
        ...
        with pytest.raises(DatabaseVersionError, match="Source database version detection failed"):
            await version_detector.detect_versions()
    
    @pytest.mark.asyncio
    async def test_detect_versions_target_failure(self, version_detector, mock_database_manager,
        ...
        # Test version detection failure
        with pytest.raises(DatabaseVersionError, match="Target database version detection failed"):
            await version_detector.detect_versions()
    ...
    async def test_check_compatibility_supported_versions(self, version_detector):
        ...
        compatibility = await version_detector.check_compatibility(source_version, target_version)
    ...
    async def test_check_compatibility_feature_differences(self, version_detector):
        ...
        assert "incremental_sorting" in feature_names or "multirange_types" in feature_names
    
    @pytest.mark.asyncio
    async def test_get_version_report(self, version_detector, mock_database_manager,
                                    mock_source_connector, mock_target_connector):
        ...
        mock_database_manager.return_source_connection = Mock()
        mock_database_manager.return_target_connection = Mock()
        mock_database_manager.get_connection_info = AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_validate_operation_support(self, version_detector, mock_database_manager,
                                            mock_source_connector, mock_target_connector):
    ...
    @pytest.mark.asyncio
    async def test_get_cached_versions_populated(self, version_detector, mock_database_manager,
                                               mock_source_connector, mock_target_connector):
    ...
    @pytest.mark.asyncio
    async def test_get_detection_statistics_populated(self, version_detector, mock_database_manager,
                                                    mock_source_connector, mock_target_connector):
...
class TestVersionDetectorEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_version_detection(self, version_detector, mock_database_manager,
                                              mock_source_connector, mock_target_connector):
    ...
    @pytest.mark.asyncio
    async def test_version_detection_with_connection_timeout(self, version_detector, mock_database_manager):
    ...
    @pytest.mark.asyncio
    async def test_malformed_version_string(self, version_detector, mock_database_manager):
        ...
        mock_database_manager.get_source_connection = AsyncMock(return_value=mock_connector)
