...
class TestRetryManager:
    ...
    def test_calculate_delay_without_jitter(self):
        ...
        config = RetryConfig(
            base_delay=2.0, backoff_factor=3.0, max_delay=20.0, jitter=False
        )
...
class TestRetryDecorator:
    ...
    def test_function_with_custom_exceptions(self):
        ...
        @retry_on_error(
            max_attempts=2, base_delay=0.01, retriable_exceptions=(ConnectionError,)
        )
    ...
    def test_function_with_before_retry_callback(self):
        ...
        @retry_on_error(max_attempts=2, base_delay=0.01, before_retry=callback_mock)
...
class TestRetryEdgeCases:
    ...
    def test_very_large_backoff(self):
        ...
        config = RetryConfig(
            base_delay=1.0, backoff_factor=10.0, max_delay=5.0, jitter=False
        )
        manager = RetryManager(config)
    ...
    def test_jitter_edge_values(self):
        ...
        config = RetryConfig(
            base_delay=2.0, jitter=True, jitter_range=(1.0, 1.0)  # No actual jitter
        )
        manager = RetryManager(config)
    ...
    def test_exception_in_before_retry_callback(self, mock_sleep):
        ...
        config = RetryConfig(before_retry=failing_callback, base_delay=0.01)
        manager = RetryManager(config)
