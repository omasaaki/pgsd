...
from pgsd.exceptions.base import (
    PGSDError, PGSDWarning, ErrorSeverity, ErrorCategory
)
...
class TestPGSDError:
    ...
    def test_error_with_original_error(self):
        ...
        assert error.technical_details["original_error_message"] == "Network unavailable"
    ...
    def test_to_json_serialization(self):
        ...
        error = PGSDError(
            "Test error",
            error_code="TEST_JSON",
            technical_details={"number": 42}
        )
...
class TestErrorEdgeCases:
    ...
    def test_circular_reference_in_technical_details(self):
        ...
        error = PGSDError(
            "Test error",
            technical_details={"mock_object": mock_obj}
        )
...
class TestErrorPerformance:
    ...
    def test_memory_usage(self):
        ...
        simple_size = sys.getsizeof(simple_error)
        ...
        complex_size = sys.getsizeof(complex_error)
        
        # Complex error should be larger but not excessively so
        assert complex_size > simple_size
        assert complex_size < simple_size * 10  # Reasonable upper bound