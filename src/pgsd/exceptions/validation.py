...
class InvalidSchemaError(ValidationError):
    ...
    def __init__(
        ...
        """
        if database:
            message = f"Schema '{schema_name}' in database '{database}' failed validation"
        else:
        ...
        if any("reference" in error.lower() or "foreign" in error.lower() for error in validation_errors):
            recovery_suggestions.append("Check foreign key references and relationships")
...
class UnsupportedFeatureError(ValidationError):
    ...
    def __init__(
        ...
        if min_supported_version:
            recovery_suggestions.append(f"Upgrade PostgreSQL to version {min_supported_version} or higher")
        
        if workaround_suggestion:
            recovery_suggestions.append(f"Workaround: {workaround_suggestion}")
        else:
            recovery_suggestions.append("Check documentation for alternative approaches")
