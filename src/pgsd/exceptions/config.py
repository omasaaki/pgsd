...
class MissingConfigurationError(ConfigurationError):
    ...
    def __init__(
        ...
        if config_section:
            message = f"Missing required {key_description} in section '{config_section}'"
        else:
        ...
        if config_file:
            if config_file.exists():
                recovery_suggestions.append(f"Edit existing configuration file: {config_file}")
            else:
                recovery_suggestions.append(f"Create configuration file: {config_file}")
        else:
            recovery_suggestions.append("Create a configuration file with required settings")
        
        if config_section:
            recovery_suggestions.append(f"Ensure section '[{config_section}]' exists in configuration")
