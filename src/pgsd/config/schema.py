...
class PGSDConfiguration:
        ...
        config_dict = {
            ...
            "postgresql": {
                ...
                "case_sensitive_identifiers": (
                    self.postgresql.case_sensitive_identifiers
                ),
            },
        }
    ...
    def __post_init__(self):
        ...
        if (
            self.source_db.host == self.target_db.host
            and self.source_db.port == self.target_db.port
            and self.source_db.database == self.target_db.database
            and self.source_db.schema == self.target_db.schema
        ):
