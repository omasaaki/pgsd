...
class ConfigurationValidator:
    ...
    def _normalize_config(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for section_name, section_data in config_dict.items():
            ...
            for key, value in section_data.items():
                ...
                elif (section_name in ["source_db", "target_db"] and 
                      key == "ssl_mode"):
    ...
    def _validate_system_config(self, system_config) -> None:
        ...
        if system_config.worker_threads > 32:
            self.logger.warning(
                f"High worker_threads ({system_config.worker_threads}) may impact performance"
            )
        
        if system_config.memory_limit_mb > 8192:
            self.logger.warning(
                f"High memory_limit_mb ({system_config.memory_limit_mb}) may impact system performance"
            )
        
        if system_config.max_connections > 50:
            self.logger.warning(
                f"High max_connections ({system_config.max_connections}) may impact database performance"
            )
    ...
    def _validate_cross_references(self, config: PGSDConfiguration) -> None:
        ...
        if (config.source_db.host == config.target_db.host and
            config.source_db.port == config.target_db.port and
            config.source_db.username == config.target_db.username and
            config.source_db.password == config.target_db.password):
        ...
        if config.comparison.max_diff_items > 10000:
            self.logger.warning(
                f"Large max_diff_items ({config.comparison.max_diff_items}) may impact performance"
            )
    ...
    def validate_runtime_requirements(self, config: PGSDConfiguration) -> List[str]:
        ...
        try:
            ...
            if free_space_mb < 100:  # Less than 100MB
                warnings.append(f"Low disk space in output directory: {free_space_mb:.1f}MB available")
        except Exception:
        ...
        if config.system.memory_limit_mb > available_memory_mb * 0.8:
            warnings.append(
                f"Memory limit ({config.system.memory_limit_mb}MB) may exceed available memory "
