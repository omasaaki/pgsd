...
class HealthMonitor:
    ...
    async def check_connection_health(self, connector: DatabaseConnector) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                health_result['checks']['connection_test'] = await connector.test_connection()
            ...
            try:
                ...
                health_result['checks']['permissions_check'] = permissions.has_required_permissions()
                ...
                health_result['accessible_schemas'] = len(permissions.accessible_schemas)
            ...
            try:
                ...
                health_result['checks']['query_test'] = len(result) > 0 and result[0].get('test') == 1
            ...
            self.logger.log(log_level, "Connection health check completed", extra={
                'connection_id': connector.connection_id,
    ...
    async def check_pool_health(self, pool: ConnectionPool) -> PoolHealth:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug("Pool connection test completed", extra={
                    'test_passed': test_result,
                    'test_time_ms': connection_test_time
                })
                
            except Exception as e:
                self.logger.warning("Pool connection test failed", extra={
                    'error': str(e)
                })
            ...
            self.logger.error("Pool health check failed", extra={
                'error': str(e)
            })
            raise DatabaseHealthError(
                f"Pool health check failed: {str(e)}",
                original_error=e
            )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'healthy_percentage': (healthy_count / total_checks) * 100 if total_checks > 0 else 0.0,
            'average_response_time': round(avg_response_time, 2),
            'longest_response_time': self._metrics['longest_response_time'],
            'shortest_response_time': self._metrics['shortest_response_time'] if self._metrics['shortest_response_time'] != float('inf') else 0.0,
    ...
    def _update_metrics(self, response_time: float, is_healthy: bool):
        ...
        total_time = self._metrics['average_response_time'] * (self._metrics['total_checks'] - 1)
        self._metrics['average_response_time'] = (total_time + response_time) / self._metrics['total_checks']
    ...
    def _calculate_trends(self, recent_checks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        first_half_avg = sum(response_times[:mid_point]) / mid_point if mid_point > 0 else 0
        second_half_avg = sum(response_times[mid_point:]) / (len(response_times) - mid_point)
        
        response_time_trend = "improving" if second_half_avg < first_half_avg else "degrading" if second_half_avg > first_half_avg else "stable"
        
        # Calculate health trend
        health_statuses = [check['is_healthy'] for check in recent_checks]
        
        first_half_healthy = sum(health_statuses[:mid_point]) / mid_point if mid_point > 0 else 0
        second_half_healthy = sum(health_statuses[mid_point:]) / (len(health_statuses) - mid_point)
        
        health_trend = "improving" if second_half_healthy > first_half_healthy else "degrading" if second_half_healthy < first_half_healthy else "stable"
        
        return {
            'response_time_trend': response_time_trend,
            'health_trend': health_trend,
            'response_time_change': round(second_half_avg - first_half_avg, 2),
            'health_rate_change': round((second_half_healthy - first_half_healthy) * 100, 2)
        }
