...
class DatabaseConnector:
    ...
    @retry_on_error(
        max_attempts=3,
        base_delay=1.0,
        retriable_exceptions=(DatabaseQueryError,)
    )
    async def execute_query(
        ...
        if not self.connection or self.connection.closed:
            raise DatabaseConnectionError(
                ErrorMessages.CONNECTION_NOT_AVAILABLE,
                connection_id=self.connection_id
            )
        ...
        try:
            with self.connection.cursor(cursor_factory=RealDictCursor) as cursor:
                # Set query timeout
                cursor.execute(f"SET statement_timeout = {ConnectionTimeout.DEFAULT_QUERY_TIMEOUT * 1000}")
            ...
            self.logger.error("Unexpected error during query execution", extra={
                "connection_id": self.connection_id,
                "query": query[:100] + "..." if len(query) > 100 else query,
                "error": str(e),
                "execution_time": execution_time
            ...
            self.logger.debug(LogMessages.QUERY_EXECUTED, extra={
                "connection_id": self.connection_id,
                "query": query[:100] + "..." if len(query) > 100 else query,
                "execution_time": execution_time
            })
    ...
    async def get_version(self) -> PostgreSQLVersion:
        ...
        try:
            ...
            self.logger.info(LogMessages.VERSION_DETECTED, extra={
                "connection_id": self.connection_id,
    ...
    async def check_permissions(self) -> DatabasePermissions:
        ...
        try:
            ...
            try:
                ...
                permissions.can_read_schema = result[0].get('has_schema_privilege', False)
            except Exception:
            ...
            try:
                ...
                permissions.can_read_tables = result[0].get('has_table_privilege', False)
            except Exception:
            ...
            try:
                ...
                permissions.can_read_constraints = result[0].get('has_table_privilege', False)
            except Exception:
                permissions.can_read_constraints = False
            
            # Get accessible schemas
            try:
                schema_result = await self.execute_query(QueryConstants.GET_ACCESSIBLE_SCHEMAS)
                permissions.accessible_schemas = [row['schema_name'] for row in schema_result]
            except Exception:
            ...
            self.logger.info(LogMessages.PERMISSIONS_VERIFIED, extra={
                "connection_id": self.connection_id,
    ...
    async def get_schema_objects(self, schema_name: str, object_type: str) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get {object_type}s in schema {schema_name}", extra={
                "connection_id": self.connection_id,
                "schema": schema_name,
    ...
    def close(self):
        ...
        if self.connection and not self.connection.closed:
            ...
            self.logger.info(LogMessages.CONNECTION_CLOSED, extra={
                "connection_id": self.connection_id
            })
