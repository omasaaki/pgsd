...
class ConnectionPool:
    ...
    def __init__(self, db_config: DatabaseConfig, max_connections: int = None):
        ...
        self.logger.info(LogMessages.POOL_CREATED, extra={
    ...
    def get_connection(self, timeout: Optional[float] = None) -> DatabaseConnector:
        ...
        try:
            ...
            self.logger.error("Failed to get connection from pool", extra={
                "error": str(e),
                "timeout": timeout,
    ...
    def return_connection(self, connector: DatabaseConnector) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to return connection to pool", extra={
                "error": str(e)
            })
    ...
    def _remove_connection(self, pooled_conn: PooledConnection) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to remove connection from pool", extra={
                "error": str(e)
            })
        ...
        with self._lock:
            ...
            if self._stats['total_created'] > 0:
                avg_connection_time = sum(
                    ...
                    for conn in self._all_connections
                ) / len(self._all_connections) if self._all_connections else 0
            else:
            ...
            for conn in self._all_connections:
                if not conn.in_use:
                    ...
                    if (conn.is_expired(self.max_lifetime) or 
                        conn.is_idle_too_long(self.idle_timeout)):
            ...
            while not self._shutdown:
                try:
                    ...
                    if not self._shutdown:
                        ...
                        self.logger.debug(LogMessages.POOL_HEALTH_CHECK, extra={
    ...
    def close(self):
        ...
        self.logger.info(LogMessages.POOL_DESTROYED, extra={
        ...
        with self._lock:
            return {
                **self._stats,
                ...
                'active_connections': sum(1 for conn in self._all_connections if conn.in_use),
                'max_connections': self.max_connections,
                'pool_utilization': (len(self._all_connections) / self.max_connections) * 100
            }
