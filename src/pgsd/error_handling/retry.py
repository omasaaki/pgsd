            ...
            try:
                ...
                if self.config.retry_on_result and self.config.retry_on_result(result):
                    if attempt < self.config.max_attempts:
                        ...
                        if self.logger:
                            self.logger.info(f"Result triggered retry, attempt {attempt + 1} in {delay:.2f}s")
                        time.sleep(delay)
                        continue
                ...
                if self.config.before_retry:
                    try:
                        ...
                        if self.logger:
                            self.logger.warning("before_retry callback failed", exc_info=True)
                
                if attempt < self.config.max_attempts:
                    delay = self.calculate_delay(attempt)
                    if self.logger:
                        self.logger.info(f"Retrying after {type(e).__name__}, attempt {attempt + 1} in {delay:.2f}s")
                    time.sleep(delay)
